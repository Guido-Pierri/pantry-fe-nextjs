...
export async function saveSearchItem(
  pantryId: number,
  name: string,
  gtin: string,
  image: string,
...
export async function saveCustomItem(
  ...
  formData: FormData,
): Promise<Promise<any> | Promise<string>> {
  const quantity = "1";
  const session = await auth();
  const token = session?.token;
...
export async function changeIsFirstTimeUser(id: string) {
  const session = await auth();
  const token = session?.token;
  const res = await fetch(
    `${apiUrl}/api/v1/users/update-first-time-user/${id}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
  ...
  console.log("changeIsFirstTimeUser Response Status :", res.status);
  if (res.status === 200) {
...
export async function updateUserProfile(
  ...
  if (typeof password === "string" && typeof confirmPassword === "string") {
    ...
    console.log("id", id);
    const res = await fetch(
      `${apiUrl}/api/v1/users/update-user-profile/${id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
